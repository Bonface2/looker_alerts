...
DASHBOARD_IDS = [
    ...
    4960, 4066, 3812, 3491, 3312, 2083, 1600
]
LOOK_IDS = [
    24762, 26488, 14245, 8727, 26006, 25739, 25389, 23834, 22393, 22211,
    22170, 20535, 20272, 19681, 19491, 18381, 17111
]
...
def build_report():
    ...
    if never_ran_looks:
        ...
        for lid, lr in never_ran_looks:
            html_parts.append(f"<li><a href='{base_url}/looks/{lid}'>Look {lid}</a> - Last run: {lr}</li>")
        html_parts.append("</ul>")
    else:
         html_parts.append("<h2><b>Looks not run in last 30 days</b></h2>")
